...
---
--- Features:
--- - Buffers are listed in the order of their identifier (see |bufnr()|) by
---   default, but can be manually reordered (see |MiniTabline.move_buffer()|
---   and `<M-,>` / `<M-.>` mappings created by |MiniTabline.setup()|).
---
...
--- # Reordering buffers ~
---
--- |MiniTabline.setup()| creates two normal mode mappings, `<M-,>` and `<M-.>`,
--- which move the current buffer's tab one position to the left or right,
--- respectively. Pressing the mapping repeatedly moves the tab further in the
--- same direction, so a buffer displayed first can be moved past a buffer
--- displayed seventh (or any other position) with repeated presses.
---
...
MiniTabline.setup = function(config)
  ...
  if vim.fn.has('nvim-0.10') == 0 then
    vim.notify(
      '(mini.tabline) Neovim<0.10 is soft deprecated (module works but is not supported).'
...
--- Move current buffer's tab in display order
---
--- Swaps the tab of the current buffer with its neighbor in the given
--- direction. Calling this repeatedly moves the tab further in that
--- direction, e.g. moving a tab from position 1 to position 7 takes six
--- calls with `direction = 1`.
---
---@param direction number Either `-1` (move tab left/earlier) or `1`
---   (move tab right/later).
MiniTabline.move_buffer = function(direction)
  ...
  -- Current buffer might not be listed (e.g. scratch buffer): nothing to move
  if cur_index == nil then return end

  local target_index = cur_index + direction
  if target_index < 1 or target_index > #H.buf_order then return end

  H.buf_order[cur_index], H.buf_order[target_index] = H.buf_order[target_index], H.buf_order[cur_index]
...
-- Manual display order of listed buffers (array of buffer identifiers)
H.buf_order = {}
...
-- Work with buffer order ------------------------------------------------------
-- Keep `H.buf_order` in sync with currently listed buffers:
-- - Buffers no longer listed (deleted/wiped/unlisted) are dropped.
-- - Newly listed buffers not yet tracked are appended at the end (in
--   ascending `buf_id` order for determinism).
H.sync_buf_order = function()
  local listed = {}
  for _, buf_id in ipairs(vim.api.nvim_list_bufs()) do
    if vim.bo[buf_id].buflisted then listed[buf_id] = true end
  end

  local new_order = {}
  for _, buf_id in ipairs(H.buf_order) do
    if listed[buf_id] then
      table.insert(new_order, buf_id)
      -- Mark as already placed so it isn't appended again below
      listed[buf_id] = nil
    end
  end

  local new_buf_ids = {}
  for buf_id, _ in pairs(listed) do
    table.insert(new_buf_ids, buf_id)
  end
  table.sort(new_buf_ids)
  for _, buf_id in ipairs(new_buf_ids) do
    table.insert(new_order, buf_id)
  end

  H.buf_order = new_order
end
